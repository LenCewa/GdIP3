...
// Globale Variablen
int sig_d, sig_r, tau, kernelSize;
...
double** Cartoonize::create2DArray(int height, int width)
{
...
void Cartoonize::destroy2DArray(double **array)
{
	free(*array);
	free(array);
}
...
Image Cartoonize::bilateralFilter(const Image &pic) 
{
...
Pixel Cartoonize::bilateralKernel(const Image &pic, int y, int x)
{
	// Hint: | y = i | x = j
	...
	int h = 0; int w = 0;
	int appliedKernelS = kernelSize / 2;
...
bool Cartoonize::inRange(int ks, int y, int x, const int height, const int width)
{ return ((y - ks / 2 < 0 || y + ks / 2 >= height) || (x - ks / 2 < 0 || x + ks / 2 >= width)) ? false : true; }

double** Cartoonize::createAndInitializeKernel(const Image &pic, int y, int x) 
{
	// Hint: | y = i | x = j
	...
	double **kernel = create2DArray(kernelSize, kernelSize);
	for (int k = y - appliedKernelS; k < y + appliedKernelS; k++) {
		for (int l = x - appliedKernelS; l < x + appliedKernelS; l++) {
...
double Cartoonize::calculateKernelField(const Image &pic, int y, int x, int k, int l) { return cDomain(y, x, k, l) * cEdge(pic, y, x, k, l); }
...
double Cartoonize::cDomain(int y, int x, int k, int l)
{
	...
	double dividend = 2 * std::pow(sig_d, 2);
	double expression = -(summand1 + summand2) / dividend;
	return std::exp(expression);
}

double Cartoonize::cEdge(const Image &pic, int y, int x, int k, int l)
{
	// Potenzregel benutzen und e^-(...) berechnen
	...
	double nenner = 2 * std::pow(sig_r, 2);
	double expression = -(zaehler) / nenner;
	return std::exp(expression);
}

double Cartoonize::euklidNorm(int r, int g, int b) { return std::sqrt((r * r) + (g * g) + (b * b)); }
...
Image Cartoonize::binaryTraversion(const Image &pic)
{
...
Pixel Cartoonize::binaryKernel(const Image &pic, int y, int x, char op)
{
	int h = 0; int w = 0;
...
double Cartoonize::adaptPixelColor(double rgb)
{
	if (rgb > 255.0)
		return 255.0;
	if (rgb < 0)
...
double** Cartoonize::getBlurMatrix()
{
...
double** Cartoonize::getDerriveMatrix(char v)
{
...
Image Cartoonize::edgeStrength(Image &picX, Image &picY)
{
...
Pixel Cartoonize::calcEdgeStrengthPixelwise(int y, int x, Image &picX, Image &picY)
{
	double r, g, b;
...
Image Cartoonize::edgeDetection(Image &pic)
{
	const int H = pic.height();
	const int W = pic.width();
	Image result = Image(H, W);
...
Pixel Cartoonize::calcEdgeDetectionPixelwise(int y, int x, Image &pic)
{
	double r, g, b;
	const Pixel &pixel = pic[y][x];

	r = (tau >= pixel.r) ? pixel.r : 0.0;
	g = (tau >= pixel.g) ? pixel.g : 0.0;
	b = (tau >= pixel.b) ? pixel.b : 0.0;
...
Image Cartoonize::cartoonize(Image &bil, Image &bin)
{
	// Sollten gleich sein, aber man kann ja nie wissen... 
...
Pixel Cartoonize::compare(const Pixel &pBil, const Pixel &pBin)
{
	double r, g, b;
	r = pBin.r;
	g = pBin.g;
