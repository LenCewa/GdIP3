#include "Richardson_Lucy_Algo.h"

#include <omp.h>
#include <cmath>
#include <ctime>

void Richardson_Lucy_Algo::process(const Parameters &params, const Image &src, Image &dst)
{
...
Image Richardson_Lucy_Algo::filter(double kernel, const Image &src)
{
...
bool Richardson_Lucy_Algo::inRange(int x, const int width) { return (x - 25 < 0 || x + 25 > width) ? false : true; }

// Wendet den 1 x 51 Kernel auf den gegebenen Pixel an und gibt die Adresse des modifizierten Pixel zurück
Pixel Richardson_Lucy_Algo::kernelOperation(double kernel, const Image &src, int y, int x)
{
...
Image Richardson_Lucy_Algo::elementwiseArithmeticOperation(const Image &left, const Image &right, char op) {
	// Dimension holen
